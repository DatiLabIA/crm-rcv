 ...
 */
class Rcvapi extends DolibarrApi
{
	/**
	 * @var Societe $company
	 */
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('rcv', 'patient', 'read')) {
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('rcv', 'patient', 'write')) {
			throw new RestException(403, 'Not allowed');
		}
	...
	public function updatePatient($id, $request_data = null)
	{
		...
		$result = $this->company->update($this->company->id, DolibarrApiAccess::$user);
		if ($result <= 0) {
			$errors = ($this->company->error ? $this->company->error : implode(', ', $this->company->errors));
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('rcv', 'consultation', 'read')) {
			throw new RestException(403, 'Not allowed');
		}
	...
	{
		if (!DolibarrApiAccess::$user->hasRight('rcv', 'consultation', 'write')) {
			throw new RestException(403, 'Not allowed');
		}
