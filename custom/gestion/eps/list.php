...
$sortfield = GETPOST('sortfield', 'aZ09comma') ?: 'e.codigo';
...
$sql = "SELECT e.rowid, e.codigo, e.descripcion, e.datec FROM ".MAIN_DB_PREFIX."gestion_eps as e WHERE e.entity = ".$conf->entity;
if ($search_codigo) $sql .= natural_search('e.codigo', $search_codigo);
if ($search_descripcion) $sql .= natural_search('e.descripcion', $search_descripcion);
...
llxHeader('', 'EPS', '');
$param = ($search_codigo ? '&search_codigo='.urlencode($search_codigo) : '').($search_descripcion ? '&search_descripcion='.urlencode($search_descripcion) : '');
$newbtn = $user->hasRight('gestion', 'write') ? '<a class="butActionNew" href="card.php?action=create"><span class="fa fa-plus-circle"></span></a>' : '';

print load_fiche_titre('EPS - Entidades Promotoras de Salud', $newbtn, 'generic');
...
print '<tr class="liste_titre_filter"><td class="liste_titre"><input class="flat maxwidth100" type="text" name="search_codigo" value="'.dol_escape_htmltag($search_codigo).'"></td><td class="liste_titre"><input class="flat maxwidth200" type="text" name="search_descripcion" value="'.dol_escape_htmltag($search_descripcion).'"></td><td class="liste_titre"></td><td class="liste_titre center"><input type="image" src="'.img_picto('','search.png','',0,1).'" name="button_search"></td></tr>';
print '<tr class="liste_titre">';
print_liste_field_titre("Código", $_SERVER["PHP_SELF"], "e.codigo", "", $param, "", $sortfield, $sortorder);
print_liste_field_titre("Descripción", $_SERVER["PHP_SELF"], "e.descripcion", "", $param, "", $sortfield, $sortorder);
print_liste_field_titre("DateCreation", $_SERVER["PHP_SELF"], "e.datec", "", $param, "", $sortfield, $sortorder);
...
while ($i < min($num, $limit)) {
    ...
    print '<tr class="oddeven"><td><a href="card.php?id='.$obj->rowid.'">'.dol_escape_htmltag($obj->codigo).'</a></td><td>'.dol_escape_htmltag($obj->descripcion).'</td><td>'.dol_print_date($db->jdate($obj->datec), 'day').'</td><td class="center nowraponall">';
...
if ($num == 0) print '<tr class="oddeven"><td colspan="4" class="opacitymedium">No hay registros</td></tr>';
print '</table></div>';
