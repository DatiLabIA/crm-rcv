...
class Programa extends GestionCommon
{
    public $element = 'programa';
    public $table_element = 'gestion_programa';
    public $nombre;
    ...
    {
        $sql = "SELECT rowid, nombre, datec, tms, fk_user_creat, fk_user_modif, entity FROM ".MAIN_DB_PREFIX.$this->table_element." WHERE rowid = ".(int)$id;
        $resql = $this->db->query($sql);
        if ($resql && $this->db->num_rows($resql)) {
            $obj = $this->db->fetch_object($resql);
            $this->id = $obj->rowid; $this->nombre = $obj->nombre;
            $this->datec = $this->db->jdate($obj->datec); $this->tms = $this->db->jdate($obj->tms);
