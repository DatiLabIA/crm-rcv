...
class ActionsCabinetMedFix
{
	...
	public function tabContentViewOrder($parameters, &$object, &$action, $hookmanager)
	{
		...
		$html .= '          $("#cmfix-load-msg").css("color","#3a3").text("Cargado. Recargando...");'."\n";
		$html .= '          window.location.reload();'."\n";
		$html .= '        } else {'."\n";
		$html .= '          btn.prop("disabled", false);'."\n";
		$html .= '          $("#cmfix-load-msg").css("color","#c55").text(data.error || "Error desconocido");'."\n";
		$html .= '        }'."\n";
		$html .= '      },'."\n";
		$html .= '      error: function(xhr) {'."\n";
		$html .= '        btn.prop("disabled", false);'."\n";
		$html .= '        try { var d = JSON.parse(xhr.responseText); $("#cmfix-load-msg").css("color","#c55").text(d.error || "Error " + xhr.status); } catch(e) { $("#cmfix-load-msg").css("color","#c55").text("Error " + xhr.status); }'."\n";
		$html .= '      }'."\n";
		$html .= '    });'."\n";
		$html .= '  });'."\n";
		$html .= '})(jQuery);'."\n";
